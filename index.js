...
var SRV_PORT = 8070;
//var DB_NAME = ':memory:';
var DB_NAME = 'smartasyno.db';

var MQTT_ADDRESS = 'localhost';
var MQTT_PORT = 1883;
var MQTT_CLIENT = 'smartasyno_server';
...
db.serialize(function() {
	db.run("CREATE TABLE IF NOT EXISTS voltage (time INT, value INT)");
	db.run("CREATE TABLE IF NOT EXISTS current (time INT, value INT)");
...
app.use(express.static('asyno'));
app.get('/', express.static('asyno'));
	...
	try {
		...
		if (m.data !== undefined) {
			...
			if (m.data.adc3 !== undefined) {
				...
				//val = Math.floor((parseInt(m.data.adc3.toString())-150)/10+21);
				db.run("INSERT INTO " + t + " VALUES ("+now()+", "+val+")");
			}
...
function get(req, res) {
	...
	try {
		switch (p.value) {
			case "latest":
/**/
				db.get("SELECT voltage.value as Vv, voltage.time as Vt, \
					current.value as Iv, current.time as It, \
					luminosity.value as Lv, luminosity.time as Lt FROM voltage \
					LEFT JOIN current ON current.time = (SELECT max(time) FROM current) \
					LEFT JOIN luminosity ON luminosity.time = (SELECT max(time) FROM luminosity) \
					WHERE voltage.time=(SELECT max(time) FROM voltage)", function(err, row) {
					...
					if (row !== undefined) {
						...
						data.history.current = {time : row.It, value: row.Iv};				
						data.history.voltage = {time : row.Vt, value: row.Vv};
			...
			case "data":
				var mode = '';
				switch (p.mode) {
					case 'current': case 'voltage': case 'luminosity': mode = p.mode; break;
					default:
